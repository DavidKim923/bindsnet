    ...
    def __init__(
        ...
        **kwargs,
        ...
        if nu is None:
            ...
            if len(nu[0]) != len(nu[1]):
                if len(nu[0]) < len(nu[1]):
                    padded_tensor0 = F.pad(nu[0], (0, np.abs(len(nu[1])-len(nu[0]))), "constant", 0)
                    self.nu = torch.stack([padded_tensor0, nu[1]], dim=0)
                else:
                    padded_tensor0 = F.pad(nu[1], (0, np.abs(len(nu[1])-len(nu[0]))), "constant", 0)
                    self.nu = torch.stack([nu[0], padded_tensor0], dim=0)
            else:
...
class PSTDP(LearningRule):
    """
    Simple PSTDP rule involving both pre- and post-synaptic spiking activity.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # language=rst
        """
        Constructor for ``PSTDP`` learning rule.

        :param connection: An ``AbstractConnection`` object whose weights the
            ``PSTDP`` learning rule will modify.
        :param nu: Single or pair of learning rates for pre- and post-synaptic events. It also
            accepts a pair of tensors to individualize learning rates of each neuron.
            In this case, their shape should be the same size as the connection weights.
        :param reduction: Method for reducing parameter updates along the batch
        ...
        self.neuron_ltime = torch.zeros(self.target.n)
        self.neuron_ltrace = torch.zeros(self.target.n)
        self.t = 0

    def _connection_update(self, **kwargs) -> None:
        # language=rst
        """
        PSTDP learning rule for ``Connection`` subclass of ``AbstractConnection``
        class.
        """
        # print("PSTDP learning rule")

        
        target_s = self.target.s.squeeze(0).to(torch.float32)
        source_s = self.source.s.squeeze(0).to(torch.float32)
        
        target_s_mask = target_s == 1
        source_s_mask = source_s == 1
        target_s_i = target_s_mask.nonzero(as_tuple=True)[0]
        source_s_i = source_s_mask.nonzero(as_tuple=True)[0]
        
        # presynaptic event
        if source_s_i.numel() != 0:
            source_time = self.neuron_ltime[source_s_i]
            target_time = self.neuron_ltime
            source_trace = self.neuron_ltrace[source_s_i]
            
            diff = target_time.unsqueeze(0) - source_time.unsqueeze(1)
            valid = diff > 0
            
            source_idx, target_idx = valid.nonzero(as_tuple=True)
            # valid_t = torch.unique(target_idx)
            
            valid_diff = diff[source_idx, target_idx]
            pap_updates = source_trace[source_idx] * torch.exp(-valid_diff / self.source.tc_trace)
            
            pap_update = torch.zeros_like(diff)
            pap_update.index_put_((source_idx, target_idx), pap_updates, accumulate=False)
            
            pbp_update = self.neuron_ltrace*torch.exp((target_time - self.t) / self.source.tc_trace)
            # mask = torch.zeros_like(pbp_update, dtype=torch.bool)
        ...
        # postsynaptic event
        if target_s_i.numel() != 0:
            target_x = self.target.x.squeeze(0)
            self.neuron_ltime[target_s_i] = self.t
            self.neuron_ltrace[target_s_i] = target_x[target_s_i]
            
        super().update()
        
        self.t += self.source.dt


class STDPPLUS(LearningRule):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # language=rst
        """
        Constructor for ``STDP+`` learning rule.

        :param connection: An ``AbstractConnection`` object whose weights the
            ``STDP+`` learning rule will modify.
        :param nu: Single or pair of learning rates for pre- and post-synaptic events. It also
            accepts a pair of tensors to individualize learning rates of each neuron.
            In this case, their shape should be the same size as the connection weights.
        :param reduction: Method for reducing parameter updates along the batch
    ...
    def _connection_update(self, **kwargs) -> None:
        # language=rst
        """
        STDP+ learning rule for ``Connection`` subclass of ``AbstractConnection``
        class.
        """
        ...
        if self.nu[0].any():
            ...
            exc_source_s_i = source_s_i[source_s_i<exc_num]
            inh_source_s_i = source_s_i[source_s_i>=exc_num]
            ...
            if len(source_s_i)!= 0:
                ...
                # exc update
                exc_weight = self.connection.w[exc_source_s_i] - target_x
                exc_weight = torch.clamp(exc_weight, min=self.connection.wmin, max=self.connection.wmax)
                self.connection.w[exc_source_s_i] = exc_weight
                # self.connection.w[exc_source_s_i] = self.connection.w[exc_source_s_i].sub_(target_x).clamp_(min=self.connection.wmin, max=self.connection.wmax)
                # inh update
                # self.connection.w[inh_source_s_i] = self.connection.w[inh_source_s_i].sub_(target_x).clamp_(min=-self.connection.wmax, max=self.connection.wmin)
        ...
        if self.nu[1].any():
            ...
            exc_target_s_i = target_s_i[target_s_i<exc_num]
            inh_target_s_i = target_s_i[target_s_i>=exc_num]
            ...
            if len(target_s_i) != 0:
                ...
                # exc update
                exc_weight = self.connection.w.T[exc_target_s_i] + source_x
                exc_weight = torch.clamp(exc_weight, min=self.connection.wmin, max=self.connection.wmax)
                self.connection.w.T[exc_target_s_i] = exc_weight
                # self.connection.w.T[exc_target_s_i] = self.connection.w.T[exc_target_s_i].add_(source_x).clamp_(min=self.connection.wmin, max=self.connection.wmax)
                
                # inh update
                inh_weight = self.connection.w.T[inh_target_s_i] + source_x
                inh_weight = torch.clamp(inh_weight, min=-1*self.connection.wmax, max=self.connection.wmin)
                self.connection.w.T[inh_target_s_i] = inh_weight
                # self.connection.w.T[inh_target_s_i] = self.connection.w.T[inh_target_s_i].add_(source_x).clamp_(min=-1*self.connection.wmax, max=self.connection.wmin)
...
class STDPPLUSE(LearningRule):
    """
    Simple STDP+ rule for excitatory synapse involving both pre- and post-synaptic spiking activity.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # language=rst
        """
        Constructor for ``STDP+`` learning rule for excitatory synapse.
    ...
    def _connection_update(self, **kwargs) -> None:
        ...
        if self.nu[1].any():
            ...
            source_x = self.source.x.squeeze(0)*self.nu[1][:len(self.source.x.squeeze(0))]
            target_s = self.target.s.squeeze(0).float()
            target_s_i = (target_s==1).nonzero().squeeze(1)
            self.connection.w.T[target_s_i] = self.connection.w.T[target_s_i].add_(source_x).clamp_(min=self.connection.wmin, max=self.connection.wmax)
            
            del target_s, source_x
            
        super().update()
    
class STDPPLUSI(LearningRule):
    """
    Simple STDP+ rule for inhibitory synapse involving both pre- and post-synaptic spiking activity.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # language=rst
        """
        Constructor for ``STDP+`` learning rule for inhibitory synapse.
